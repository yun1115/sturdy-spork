...
def codes(calls):
	...
	for a in calls:
		if a[0][0:5] == '(080)':
			called_number.append(a[1])


	codes_all = []
	for numbers in called_number:
		if numbers[0] == '(':
			codes_all.append(numbers[1:numbers.find(')') ])
		elif numbers[0:3] == '140':
			codes_all.append('140')
		else:
			codes_all.append(numbers[0:4])

	codes_only = []
	for code in codes_all:
		if code not in codes_only:
			codes_only.append(code)
